...
global UIA := UIA_Interface() ; UIA Used as an alternate method for grabbing text, Credit to Descolada @ https://github.com/Descolada/UIAutomation
global mEl := ; UIA Element used for grabbing text
running := false
inLoop := false
...
windowsFound := 0
 ...
 ; MAIN LOOP START-------------------------------------------------------------------------------------------------------------------------------------------------
	Loop
	{
	 ...
	 newText := GrabText(mEl, windowControl, windowControlPosition, currentTitle, currentID, windowControlVerified, windowControlTestText, loopMember, clickPos, copyPos, copyID, false, false, false)		 		
		if (running = false)
		{
		 return
		}	 
		 ...
		 partialTextUpdated := false
			...
			{ ; NEW TEXT TAKES PRECEDENCE OVER PARTIAL TEXT
			 ...
			 ; newLineTextPrev := ""
				...
				; {
				 ; newLineTextPrev := newLineTextPrev . newLines[A_Index] . "`n"
				; }
			 ...
			 ; newLineText := ""
				; Loop % newLines.MaxIndex()
				; {
				 ; newLineText := newLineText . newLines[A_Index] . "`n"
				; }
			 ; previousUserPartialLineText := ""
				; Loop % newLines.MaxIndex()
				; {
				 ; previousUserPartialLineText := previousUserPartialLineText . previousUserPartialLines[A_Index] . "`n"
				; }
			 ;MsgBox % lineDifference "`n`n" newLineText "`n`n" newLineTextPrev "`n`n" previousUserPartialLineText "`n`n" newLines[newLines.MaxIndex()]
					...
					Loop, % breakPoints.MaxIndex() - 1
					{
					 ...
					 bp2 := breakPoints[A_Index + 1]
						...
						else if (pos > bp) && (pos < bp2)
						{
						 ...
						 ;MsgBox % A_Index " " insertionPoint " " bp " " pos " " char " " charNext " " breakPoints.MaxIndex()
...
WatchCursor:
{
 ...
 rectangleGuideColor := "Blue"
	...
	if (settingTextOutputWindow = true)
	{
	 ...
	 mEl := UIA.SmallestElementFromPoint(mX, mY, true, mHwnd) ; Deep search enabled by default
		if (IsObject(mEl))
		{
