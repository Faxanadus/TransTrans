...
scriptName := SubStr(A_ScriptName, 1, StrLen(A_ScriptName) - 4)
	...
	Loop %maxMembers%
	{
	 ...
	 windowCopyIDs.Push("")
	 windowElements.Push("")
	 windowElementXPos.Push(0)
	 windowElementYPos.Push(0)
	 windowElementIDs.Push(0)
	 firstSamples.Push(1)
	...
	if (settingTextOutputWindow = true)
	{
	 mEl := UIA.SmallestElementFromPoint(mX, mY, true, id) ; UIA Deep search enabled by default
		if (IsObject(mEl))
		{
		 ...
		 windowElementXPos[currentMember] := mX
		 windowElementYPos[currentMember] := mY
...
SaveSettings()
{
 global
	Loop %maxMembers%
	{
	 ...
	 windowElementXPosVar .= windowElementXPos[A_Index] . ((A_Index < maxMembers) ? (",") : (""))
	 windowElementYPosVar .= windowElementYPos[A_Index] . ((A_Index < maxMembers) ? (",") : (""))
 ...
 iniFile.Set("windowElementXPos", "Participant Windows", windowElementXPosVar)
 iniFile.Set("windowElementYPos", "Participant Windows", windowElementYPosVar)
...
global
 ...
 windowElementXPos := StrSplit(iniFile.Get("windowElementXPos", "Participant Windows"), ",")
 windowElementYPos := StrSplit(iniFile.Get("windowElementYPos", "Participant Windows"), ",")
 windowElementIDs := StrSplit(iniFile.Get("windowElementIDs", "Participant Windows"), ",")
 
 	Loop, % maxMembers
	{
		if (windowIDs[A_Index] != "") && (windowElementIDs[A_Index] != 0) && (windowElementIDs[A_Index] != "") 
		{ 
