...
logFileTimestampsFromStart := true
...
lineIDType := 1 ; OPTIONS FOR SENDING A UNIQUE IDENTIFIER FOR EACH LINE TO ANOTHER APPLICATION, AS AN INT (BIG ENDIAN OR LITTLE ENDIAN) OR WITHIN THE STRING AS {ID}
...
scriptTitle := "Trans/Trans"
scriptTitleExtended := "Transcription/Translation"
displayLogFileName := "TransTrans-DisplayLog"
existingScriptFound := false

WinGet, ExistingScript,, %scriptTitle%
WinGet, ExistingScriptExtended,, %scriptTitleExtended%
scriptNumber := 1
	if (ExistingScript || ExistingScriptExtended)
	{
	 existingScriptFound := true
		while (existingScriptFound)
		{
		 scriptNumber += 1
		 scriptTitle := % "Trans/Trans " scriptNumber
		 scriptTitleExtended := % "Transcription/Translation " scriptNumber
		 WinGet, ExistingScript,, %scriptTitle%
		 WinGet, ExistingScriptExtended,, %scriptTitleExtended%
			if (ExistingScript || ExistingScriptExtended)
			 existingScriptFound := true
	 ...
	 displayLogFileName := % "TransTrans-DisplayLog" scriptNumber
	}
...
currentLanguage := GetOSLanguage()
	...
	Loop %maxMembers%
	{
	 ...
	 userLineTimes.Push(currentUserLineTimes)
	 userLineStartTimes.Push(0)
	 firstWords.Push("")
...
MemberEdit_TTJa := "現在の参加者数。異なる転写ソースを持つ最大 9 人の参加者を有効にできます。"
...
MemberEnabledCheck_TTJa := "チェックすると、このメンバーの転写されたテキストを表示または記録できます。"
...
TranslateEnabledCheck_TTJa := "チェックすると、このメンバーの翻訳されたテキストを表示または記録できます。"

Gui, Add, Button, x152 y10 w80 h18 gShowAppSetupButton, App Setup >>
Gui, Add, Button, x152 y31 w80 h18 gShowAdvSetupButton, Options  >>
Gui, Add, Button, x152 y52 w80 h18 gNetwork, Network >>
...
ShowNameCheck_TTJa := "チェックすると、表示テキストとともにこのメンバーの名前が表示されます。"
...
WindowGetButton_TTJa := "このボタンをクリックして、この参加者の出力テキスト ソースの場所を選択します。"

Gui,Font, BOLD
Gui, Add, Text,x10 y176, Save Transcription to File:
Gui,Font
Gui, Add, CheckBox, x164 y176 gFileEnabledCheck vFileEnabledCheck Checked0, Enabled
...
FileEnabledCheck_TTJa := "チェックすると、以下の選択されたディレクトリに転写ログ テキスト ファイルが作成および更新されます。`nこれは、TransDisplayLog.txt 表示ファイル (起動時にこのスクリプトのディレクトリに作成される) とは別のものです。"

Gui, Add, Edit, x10 y194 r1 vFileEdit w220 +ReadOnly, Default Set: Saved in this script's directory.
Gui, Add, Button, x10 y220 w50 h18 vBrowseButton gBrowseButton,  Browse
Gui, Add, Button, x152 y220 w80 h18 gFileOptionsButton, File Options >>
...
global iniFile := new ini(iniFileDir) 
	if (FileExist(iniFileDir))
	 LoadSettings()
Gui, Show, %mainWindowPos% w240 h286, %scriptTitle%
displayFile := directory "\" displayLogFileName ".txt"
...
StartButton:
{
	if (running)
	 return
PreStartCheck:
transcriptionStartTime := A_TickCount
inLoop := false
running := false
enabledCount := 0
titleSet := 0
windowsFound := 0
 ...
 ; MAIN LOOP START-------------------------------------------------------------------------------------------------------------------------------------------------
	Loop
	{
	 ...
	 currentUserLineTimes := userLineTimes[loopMember]
	 currentUserLineStartTime := userLineStartTimes[loopMember]
	 ...
	 FoundPos := InStr(newText, "    >>")
		...
		{ ; BEGIN COMPARING PREVIOUS RECEIVED TEXT TO THE NEW TEXT
		 FoundPos := InStr(newText, previousText)
			...
			if (currentUserLineStartTime <= 0)
			 currentUserLineStartTime := A_TickCount
			...
			{ ;PREVENTS THE FIRST FOUND TEXT FROM BEING A GIGANTIC WALL OF TEXT
			 ...
			 firstSample = false
				...
				if (translationOutputControl != "") && (translationOutputWindow != "")
				 ControlSetText, %translationOutputControl%,, %translationOutputWindow% ; CLEARS THE TRANSLATION OUTPUT THE FIRST TIME TO PREVENT GRABBING EXISTING CONTENT
			 Goto, EndLoop
			}
		...
		if (StrLen(newLine) > 2)
		{
		 ...
		 partialTextUpdated := false
			 ...
			 maxPrevLines := Floor(previousUserPartialLines.MaxIndex())
				...
				{ ; ALWAYS UPDATE THE LAST LINE FROM PARTIAL TEXT
				 lineDifference := maxNewLines - 1
					Loop, % lineDifference
					 newLines.RemoveAt(1) ; REMOVE LINES THAT HAVE ALREADY BEEN DISPLAYED BY PARTIAL TEXT
				}
				else
				{
			...
			{
			 currentUserLines.Push(newLines[A_Index]) ; ADD NEW LINES
			 currentUserLineTimes.Push(currentUserLineStartTime)
			}
		...
		if (currentUserLines.MaxIndex() > 0)
		{
			if (currentUserLines[1] = "")
			{
			 ...
			 currentUserLineTimes.RemoveAt(1)
			}
		 thisUserLine := currentUserLines[1]
		 thisUserLineID := currentUserLineIDs[1]
		 thisUserLineStartTime := currentUserLineTimes[1]
		}
		if (partialTranscriptions = true) && (currentUserLines.MaxIndex() <= 0) && (newPartialText != "")
		{
		 ...
		 thisUserLineStartTime := currentUserLineTimes[partialLineIndicator]
			if (debug)
			 GuiControl, debugWindow:, PartialTextEdit, % thisUserLineID " " thisUserLineStartTime " " thisUserLine
		}
		...
		{ ;PASTE THE NEW LINE OF TEXT IN THE TRANSLATOR OR OUTPUT AREA
			...
			else if (currentLinesPartial && partialLineIndicator = 1) ; CAPITALIZE FIRST LETTER
			 thisUserLine := RegExReplace(thisUserLine, "^(\s*)(.)", "$1$u2")
			
			if (debug)
			 GuiControl, debugWindow:, DisplayLineEdit, % currentUserLineStartTime " " A_TickCount "`n" thisUserLine
			...
			if (StrLen(thisUserLine) > 2) || (lineComplete = true)
			{
				...
				if (lastLineID > 0)
				 currentLines[lastLineID] := finalText ;MODIFY THE CURRENT LINE FOR THE USER IF IT EXISTS
				else 
				...
				{ ;WRITE TEXT TO LOG FILE
				 ...
				 directoryString := directory "\" CurrentDateTime " Transcript.txt"
					if (logFileTimestamps = true)
					{
						if (logFileTimestampsFromStart = true)
						{
						 startTime := currentUserLineStartTime - transcriptionStartTime
						 ms := Mod(startTime, 1000) ; milliseconds
						 startTime := Floor(startTime / 1000) ; convert to seconds
						 sec := Mod(startTime, 60) ; seconds
						 startTime := Floor(startTime / 60) ; convert to minutes
						 min := Mod(startTime, 60) ; minutes
						 startTime := Floor(startTime / 60) ; convert to hours
						 hrs := startTime ; hours
						 StartTimeString := Format("{:02}:{:02}:{:02}.{:03}", hrs, min, sec, ms)

						 transTime := A_TickCount - transcriptionStartTime
						 ms := Mod(transTime, 1000) ; milliseconds
						 transTime := Floor(transTime / 1000) ; convert to seconds
						 sec := Mod(transTime, 60) ; seconds
						 transTime := Floor(transTime / 60) ; convert to minutes
						 min := Mod(transTime, 60) ; minutes
						 transTime := Floor(transTime / 60) ; convert to hours
						 hrs := transTime ; hours
						 formattedTime := Format("{:02}:{:02}:{:02}.{:03}", hrs, min, sec, ms)
						 TimeString := StartTimeString " --> " formattedTime "`n"
						}
						else
						 FormatTime, TimeString, A_Now, [hh:mm:ss tt]
						 
						if (separateLogFile = true)
						{
						 finalTranscription := % TimeString thisUserLine
						 finalTranslation := % TimeString transText
							if (logFileTimestampsFromStart)
							{
							 finalTranscription := % finalTranscription "`n"
							 finalTranslation := % finalTranslation "`n"
							}
						}
						else
						{
						 finalTranscription := % TimeString finalText
							if (logFileTimestampsFromStart)
							 finalTranscription := % finalTranscription "`n"
						}
					}
					else
					{
						if (separateLogFile = true)
						{
						 finalTranscription := % thisUserLine
						 finalTranslation := % transText
						}
						else
						 finalTranscription := finalText
					}
				 FileAppend, %finalTranscription%`n, %directoryString%, UTF-8
				 	if (separateLogFile = true)
					{
					 directoryString := directory "\" CurrentDateTime " Translation.txt"
					 FileAppend, %finalTranslation%`n, %directoryString%, UTF-8
					}
				}
			 lastLine := finalText
	 ...
	 userLineTimes[loopMember] := currentUserLineTimes
	 userLineStartTimes[loopMember] := currentUserLineStartTime
...
ResetAll()
{
 ...
 currentUserPartialLineIDs := []
	...
	{ ; RESET MEMBER VARIABLES
	 ...
	 userLineTimes[A_Index] := currentUserLineTimes
	 userLineStartTimes[A_Index] := 0
...
GrabText(UIAEl, control, controlPosition, window, windowID, controlVerified, controlVerificationText, controlVerificationTextID, thisClickPos, thisCopyPos, thisCopyID, skipMouseMethod, skipKeyboardMethod, forceClickOnce)
{
 ...
 forceAlternateMethod := false
	...
	if (grabbedText == "") && ((UIAEl = "") || (!IsObject(UIAEl)) || !WinExist("ahk_id" windowID))
	{ ; Prevent the program from trying to grab text from the same window another member is already using if it can't find a direct UIAE1 ID
	 sameWindow := false
	 	Loop, %maxMembers%
		{ 
			if (memberEnabled[A_Index] = true) && (windowIDs[loopMember] = windowID)
			{
			 sameWindow := true
			 break
			}
	...
	if ((UIAEl = "") || (!IsObject(UIAEl))) && ((control = "") || (controlVerified = false)) && (grabbedText = "")
	{ ; GRAB TEXT DIRECTLY USING KEYBOARD OR MOUSE METHOD
		 ...
		 waitIterations := 0
			...
			{ ; CLEARS THE RIGHT CLICK MENU IF THE LAST GRAB ATTEMPT USED THE MOUSE OR CLICKS ON THE WINDOW TO REGAIN FOCUS
			 lastControlState := false
			 	if (control != "")
				 ControlClick, %control%, ahk_id %windowID%,, Left, 1, %controlPosition%  ;CLICK THE RELATIVE POSITION WITHIN THE CONTROL AREA
				if (windowID != "") && WinExist("ahk_id" windowID)
	 ...
	 CopyEnd:
		...
		else if (grabbedText = savedClipboard)
		 return ""
		else if (grabbedText != "")
...
{ ; TURNS OFF EDIT FIELDS BECAUSE THESE WILL BUG OUT IF THE LOOP IS ACTIVE
 ...
 GUIControl, Enable%toggle%, DisplayLogFileNameEdit
...
global
 allLines := ""
	...
	{
		if FileExist(displayFile)
		 FileDelete, % displayFile ; REFORM THE DISPLAY FILE TO SHOW THE NEW CONTENT
...
ShowAppSetupButton:
{
	if (appSetupDisplay != true) && (appSetupDisplay != false)
	{
	 ...
	 TransInputGetButton_TTJa := "このボタンをクリックすると、(プロンプトが表示されたら) 通常、翻訳する単語を入力する`n翻訳アプリケーションのテキスト入力領域を直接クリックします。"
	 ...
	 TransOutputGetButton_TTJa := "このボタンをクリックして、（プロンプトが表示されたら）翻訳されたテキストが表示される翻訳アプリケーションエリアのテキスト出力エリアを直接クリックしてください。"
	 ...
	 TransButtonGetButton_TTJa := "このボタンをクリックして、(プロンプトが表示されたら) 通常クリックして翻訳を開始する翻訳アプリケーションのボタンを直接クリックします。`n 一部のアプリケーションにはこのボタンがない場合があるため、これは必要ない場合があります。"
	 ...
	 TransDisplayGetButton_TTJa := "このボタンをクリックして、(プロンプトが表示されたら) 最終的な翻訳されたテキスト出力を表示するアプリケーションの領域を直接クリックします。`n 表示する予定のテキストは、このスクリプトのディレクトリにある `nTransDisplayLog.txt という名前のファイルを代わりに使用できます。"
	...
	{
	 HideAppSetup()
	 Gui, Show, w240 h286, %scriptTitle%
	}
	else if (appSetupDisplay = false)
	{
		...
		if (networkDisplay = true)
		 HideNetwork()
		if (fileOptionsDisplay = true)
		 HideFileSetup()
	 ShowAppSetup()
	 Gui, Show, w470 h286, %scriptTitleExtended%
...
ShowAdvSetupButton:
{
	if (advancedOptionsDisplay != true) && (advancedOptionsDisplay != false)
	{
	 ...
	 PartialTranscriptionsCheck_TTJa := "チェックすると、行または文全体が完了する前に進行中の文字起こしが表示されます。`nソフトウェアによっては、自己修正するまでの最初の数秒以内に不正確な文字起こしが表示される場合があります。"
	 
	 Gui, Add, CheckBox, x240 y29 gPartialTranslationsBox vPartialTranslationsCheck Checked%partialTranslations%, Partial Translations
	 ...
	 PartialTranslationsCheck_TTJa := "部分転写」と一緒にチェックすると、完全な行または文が完成する前に進行中の翻訳も表示されます。`n正確性を考えると完全な文が最適ですが、文の断片は多くの場合十分に適切に翻訳されます。"
	 Gui,Font
	
	 Gui,Font, BOLD 
	 Gui, Add, Text, x240 y50 vTransDelayText, Translation Delay:
	 Gui,Font
	 Gui, Add, Edit, x376 y48 w42 h17 gTransDelayEdit vTransDelayEdit, % translationDelay
	 ...
	 TransDelayEdit_TTJa := "翻訳アプリが特定のメッセージを翻訳するのにかかる平均時間（ミリ秒）。`n翻訳アプリケーションが遅い場合、またはインターネット ベースの翻訳ツールを使用するときに遅延がある場合は、この遅延を増やします。"
	
	 Gui,Font, BOLD 
	 Gui, Add, Text, x240 y72 vMinDelayText, Text Sample Delay:
	 Gui,Font
	 Gui, Add, Edit, x376 y70 w42 h17 gMinDelayEdit vMinDelay, % minDelayBetweenTextGrabAttempts
	 ...
	 MinDelay_TTJa := "このアプリが 1 人の参加者の文字起こしアプリケーションからテキストを取得しようとする間隔のミリ秒数。`n複数の参加者が有効になっている場合、この遅延はアクティブな参加者の数で割られます。"
	 
	 Gui,Font, BOLD
	 Gui, Add, Text, x240 y94 w250 vNewLineText, Force New Line Delay:
	 Gui,Font
	 Gui, Add, Edit, x376 y92 w42 h17 gMinLineDelayEdit vMinLineDelayEdit, % minDelayBeforeNewLine
	 ...
	 MinLineDelayEdit_TTJa := "1 人の参加者から新しい文字起こしテキストが検出されなかった場合に、アプリが `n(現在の行に追加するのではなく) 新しい行 を作成するまで待機するミリ秒数。"
	 
	 Gui,Font, BOLD
	 Gui, Add, Text, x240 y116 vNextLineText, Line Processing Delay:
	 Gui,Font
	 Gui, Add, Edit, x376 y114 w42 h17 gNextLineEdit vNextLineDelay, % minDelayBeforeProcessingNewLine
	 ...
	 NextLineDelay_TTJa := "前の行が終了した後、このアプリが新しい行のテキストの取り込みを開始するまで待機するミリ秒数。`nこれは、部分転写が有効になっている場合に、初期結果が不正確になるのを防ぐためです。"
	 
	 Gui,Font, BOLD
	 Gui, Add, Text, x240 y138 vTotalLinesText, Total Lines to Display:
	 Gui,Font
	 Gui, Add, Edit, x376 y136 w42 h17 gLinesEdit Limit2 vMaxLines
	 ...
	 MaxLines_TTJa := "表示領域または TransDisplayLog.txt 表示ファイル (起動時にこのスクリプトのディレクトリに作成される)`nに表示されるテキストの行数。文字起こしと翻訳の両方が有効な場合、`nこれは 2 行 (言語ごとに 1 行) としてカウントされます。"
	 
	 Gui, Add, UpDown, vLinesUpDown gLinesCheck +Wrap Range1-99, % maxDisplayLines
	 
	 Gui,Font, BOLD
	 Gui, Add, Text, x240 y160 vLineTimeoutText, Line Display Timeout:
	 Gui,Font
	 Gui, Add, Edit, x376 y158 w42 h17 gTimeoutEdit vTextTimeout, % lineTimeout
	 ...
	 TextTimeout_TTJa := "古いテキスト行を表示領域または「DisplayLog.txt」表示ファイル (起動時にこのスクリプトのディレクトリに作成される)`n から削除するまでに待機するミリ秒数。これを 0 に設定すると、行はタイムアウトしないことを意味します。"
	 
	 Gui,Font, BOLD
	 Gui, Add, Text, x240 y182 vMaxCharsText, Max Characters Per Line:
	 Gui,Font
	 Gui, Add, Edit, x376 y180 w42 h17 gMaxCharsEdit vMaxCharsEdit, % maxCharactersPerLine
	 ...
	 MaxCharsEdit_TTJa := "別の行が作成されるまでの表示行あたりの最大文字数。`nこれには、参加者の名前が表示される場合にはその名前も含まれます。"
	 
	 Gui,Font, BOLD
	 Gui, Add, Text, x240 y204 vDisplayLogFileNameText, Display Log File Name:
	 Gui,Font
	 Gui, Add, Edit, x240 y222 w180 h18 gDisplayLogFileNameEdit vDisplayLogFileNameEdit, % displayLogFileName
	 
	 Gui,Font, BOLD
	 Gui, Add, CheckBox, x240 y246 gUseDisplayFileCheck vUseDisplayFileCheck Checked%useDisplayFile%, Use Display Log File
	 UseDisplayFileCheck_TT := "If checked, a file with this app's name and then TransDisplayLog.txt will be created in this script's directory,`nand will be continuously updated with participant's transcription/translations.`nThis can be used to display output in text readers such as an OBS Text (GDI+) source."
	 UseDisplayFileCheck_TTEs := "Si está marcada, se creará un archivo con el nombre de esta aplicación y luego TransDisplayLog.txt en el directorio`nde este script y se actualizará continuamente con las transcripciones/traducciones de los participantes.`nEsto se puede utilizar para mostrar la salida en lectores de texto como una fuente de texto OBS (GDI+)."
	 UseDisplayFileCheck_TTJa := "オンにすると、このアプリの名前を持つファイルと TransDisplayLog.txt がこのスクリプトのディレクトリに作成され、`n参加者の文字起こし/翻訳で継続的に更新されます。 これは、OBS テキスト ( GDI+) ソース。"
	 
	 Gui, Add, CheckBox, x240 y266 gSaveSettingsCheck vSaveSettingsCheck Checked%saveSettingsOnClose%, Save Settings on Close
	 ...
	 SaveSettingsCheck_TTJa := "チェックすると、このアプリを閉じると、すべての設定がこのスクリプトのディレクトリ内の TransTrans.ini という名前のファイルに保存されます。"
	 Gui,Font
	...
	else if (advancedOptionsDisplay = false)
	{
		...
		if (fileOptionsDisplay = true)
		 HideFileSetup()
	 ShowAdvanvedSetup()
	 Gui, Show, w427 h286, %scriptTitleExtended%
	 ToggleEditFields(!running)
	}
return
}
...
ShowAdvanvedSetup()
{
 ...
 GUIControl, Show, TransDelayEdit
 ...
 GUIControl, Show, MaxCharsText
 GUIControl, Show, MaxCharsEdit
 GUIControl, Show, DisplayLogFileNameText
 GUIControl, Show, DisplayLogFileNameEdit
...
HideAdvanvedSetup()
{
 ...
 GUIControl, Hide, TransDelayEdit
 ...
 GUIControl, Hide, MaxCharsText
 GUIControl, Hide, MaxCharsEdit
 GUIControl, Hide, DisplayLogFileNameText
 GUIControl, Hide, DisplayLogFileNameEdit
...
DisplayLogFileNameEdit:
{
 gui,submit,nohide ;updates gui variable
 displayLogFileNameString := DisplayLogFileNameEdit
 StringReplace, displayLogFileNameString, displayLogFileNameString, %A_SPACE%,, All
	if (displayLogFileNameString != "")
	 displayLogFileName := DisplayLogFileNameEdit
	else
	 displayLogFileName := "TransTrans-DisplayLog" %scriptNumber%
 return
}
...
Network:
{
	if (networkDisplay != true) && (networkDisplay != false)
	{
	 ...
	 ReceiveMessages_TTJa := "チェックした場合、以下のIPアドレスとポートからのメッセージを受信し、翻訳します。"
	 ...
	 SendMessages_TTJa := "チェックすると、翻訳された受信メッセージを以下のIPアドレスとポートに送信します。"
	 ...
	 MessageRate_TTJa := "受信した各メッセージをトランスレータに送信してから、指定した送信IPアドレス/ポートに送信するまでの遅延時間（有効な場合）。"
	 ...
	 IncludeMessageAddress_TTJa := "チェックした場合、この例のようにカンマで区切って、メッセージの前に以下のアドレスを同じパケットに含めます： `n/AHK/メッセージ、メッセージの例はこちら。 これは、OSCプロトコルでメッセージを送信するために使用できる。"
	 ...
	 SendDisplayContent_TT := "If checked, will continuously send all participant Transcribed/Translated content to the specified IP address/port.`nThis is the same content that would be output to a display area or written to the TransDisplayLog.txt file."
	 SendDisplayContent_TTEs := "Si está marcado, enviará continuamente todo el contenido transcrito/traducido de los participantes a la dirección IP/puerto especificado.`nEste es el mismo contenido que se mostraría en un área de visualización o se escribiría en el archivo TransDisplayLog.txt."
	SendDisplayContent_TTJa := "チェックすると、すべての参加者の文字起こし/翻訳されたコンテンツが指定された IP アドレス/ポートに継続的に送信されます。`nこれは、表示領域に出力されるか、TransDisplayLog.txt ファイルに書き込まれるのと同じコンテンツです。"
	 ...
	 Gui,Font
	 Gui, Add, Edit,x396 y192 w40 h18  gDisplayPortEdit vDisplayPortEdit, % displayPort
	 Gui,Font, BOLD
	 ...
	 IncludeDisplayMessageAddress_TTJa := "チェックすると、同じパケット内の参加者の表示コンテンツ テキストの前に、次の例のようにカンマで区切って以下のアドレスが含まれます: `n/AHK/Display、参加者 2: こんにちは! Hello！`nこれは、OSC プロトコル経由ですべての参加者のコンテンツを送信するために使用できます。"
	 ...
	 SendLines_TTJa := "チェックすると、更新時に表示コンテンツ全体を送信するのではなく、転記/翻訳された各テキスト行が送信されます。`n部分転記が有効な場合は、メッセージ (文字列) の前に行 ID (整数として) がパケットに含まれます。カンマで区切ります。`nこれは、受信側アプリがどの行を更新するかを知るために重要です。"
	 ...
	 LineIDTypeSelection_TTJa := "この選択ボックスは、整数の送信方法を決定します。これにより、部分文字起こしが有効になっているときに、受信側アプリがどの行が更新されているかを識別できるようになります。`nLineID as int (BE) : LineID は、文字列の前にビッグ エンディアン整数として送信されます。同じパケットをコンマで区切ります。`nLineID as int (LE) LineID は、同じパケット内の文字列の前に、リトル エンディアン整数として送信されます。同じパケット内でカンマで区切られます。`nLine {ID} in string : LineID は次のとおりです。整数として送信するのではなく、行コンテンツ内の同じ文字列の先頭に送信します。例: {6}こんにちは、これが行のテキストです。」"
	...
	else if (networkDisplay = false)
	{
		...
		if (fileOptionsDisplay = true)
		 HideFileSetup()
	 ShowNetwork()
	 Gui, Show, w444 h286, %scriptTitleExtended%
	 ToggleEditFields(!running)
	}
return
}
...
FileOptionsButton:
{
	if (fileOptionsDisplay != true) && (fileOptionsDisplay != false)
	{
	 Gui,Font, BOLD
	 Gui, Add, CheckBox, x240 y10 gTimestampCheck vTimestampCheck Checked1, Add Timestamps
	 Gui,Font
	 ...
	 TimestampCheck_TTJa := "チェックすると、上記のディレクトリに保存された転写ログにタイムスタンプが追加されます`n(起動時にこのスクリプトのディレクトリに作成される TransDisplayLog.txt 表示ファイルとは別)。"

	 Gui,Font, BOLD
	 Gui, Add, CheckBox, x240 y31 gTimestampsFromStartCheck vTimestampsFromStartCheck Checked1, Use .SRT Timestamp Format
	 Gui,Font
	 TimestampsFromStartCheck_TT := "If checked, timestamp time will be calculated from the time the START button is pressed rather than the actual time of day."
	 TimestampsFromStartCheck_TTEs := "Si está marcada, la hora de la marca de tiempo se calculará a partir del momento en que se presione el botón START en lugar de la hora real del día."
	 TimestampsFromStartCheck_TTJa := "チェックすると、タイムスタンプ時刻は実際の時刻ではなく、START ボタンが押された時刻から計算されます。"
	 
	 Gui,Font, BOLD
	 Gui, Add, CheckBox, x240 y52 gSeparateFileCheck vSeparateFileCheck Checked1, Separate File Per Language
	 Gui,Font
	 SeparateFileCheck_TT := "If checked, a separate log file will be generated for each language. With the original language log being named [Today's Date] Transcript,`nand the other language [Today's date] Translation."
	 SeparateFileCheck_TTEs := "Si está marcada, se generará un archivo de registro independiente para cada idioma. Con el registro del idioma original denominado [Fecha de hoy] Transcript`ny el otro idioma [Fecha de hoy] Translation."
	 SeparateFileCheck_TTJa := "チェックすると、言語ごとに個別のログ ファイルが生成されます。元の言語のログの名前は [今日の日付] Transcript、もう一方の言語のログは [今日の日付] 翻訳です。"
	 
	 fileOptionsDisplay := false
	}
	...
	else if (fileOptionsDisplay = false)
	{
		if (appSetupDisplay = true)
		 HideAppSetup()
		if (advancedOptionsDisplay = true)
	 ...
	 ShowFileSetup()
	 Gui, Show, w420 h286, %scriptTitleExtended%
	 ToggleEditFields(!running)
	}
return
}
...
{
 global
 fileOptionsDisplay := true
 GUIControl, Show, TimestampCheck
 GUIControl, Show, TimestampsFromStartCheck
 GUIControl, Show, SeparateFileCheck
}
...
HideFileSetup()
{
 ...
 GUIControl, Hide, TimestampsFromStartCheck
 GUIControl, Hide, SeparateFileCheck
}
...
TimestampsFromStartCheck:
gui,submit,nohide ;updates gui variable
logFileTimestampsFromStart := TimestampsFromStartCheck
return

SeparateFileCheck:
gui,submit,nohide ;updates gui variable
separateLogFile := SeparateFileCheck
return
	...
	class IniParser {
		Parse(ini_file)
		{
		 ...
		 section_name := ""
			...
			Loop, read, %ini_file%
			{ ; lines can be nothing but whitespace, we clean them up before processing any further
...
SaveSettings()
{
 ...
 iniFile.Set("logFileTimestampsFromStart", "Options", logFileTimestampsFromStart)
 iniFile.Set("separateLogFile", "Options", separateLogFile)
...
global
 ...
 logFileTimestampsFromStart := iniFile.Get("logFileTimestampsFromStart", "Options", default_value = logFileTimestampsFromStart)
 separateLogFile := iniFile.Get("separateLogFile", "Options", default_value = separateLogFile)
